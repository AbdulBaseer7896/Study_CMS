...
# ── Self-Update endpoint — any authenticated user can update their own profile/password ──
@api_view(['PATCH'])
@permission_classes([IsAuthenticated])
def me_update(request):
    """
    PATCH /api/v1/me/update/
    Any logged-in user (admin, consultant, student) can update
    their own profile picture and/or password.
    """
    user = request.user
    serializer = UserSerializer(
        user, data=request.data, partial=True,
